...
export default function KserveComponents(): JSX.Element {
  return (
    <div className="bg-gradient-to-b from-gray-100 dark:from-gray-800 to-white dark:to-gray-900 py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-4xl">KServe Components</h2>
        </div>
        <ul
          role="list"
        ...
        >
          {kserveComponents.map((component, index) => (
            <li key={index} className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md transition-transform transform hover:scale-105">
              <a href={component.link} className="block group">
                <div className="aspect-[3/2]">
                  <img className="w-full h-full object-cover" src={component.imageUrl} alt="" />
                </div>
                <div className="p-6">
                  <h3 className="mt-2 text-xl font-semibold leading-7 text-gray-900 dark:text-white group-hover:text-indigo-500">
                    {component.title}
                  </h3>
                  <p className="mt-3 text-base leading-6 text-gray-600 dark:text-gray-300">{component.subtitle}</p>
                </div>
              </a>
            </li>
          ))}
